                    ...
                    <div class="post-card">
                        <div class="post-card__image">
                            @if($post->photos && count($post->photos))
                                <img src="{{ asset('storage/'.$post->photos[0]) }}" alt="{{ $post->name ?: 'Фото объявления' }}">
                            @else
    ...
    document.querySelectorAll('.favorite-form').forEach(function (form) {
        form.addEventListener('submit', function (e) {
            ...
            .then(data => {
                ...
                const button = form.querySelector('.post-card__favorite');
                if (!button) return;

                button.classList.toggle('active', !!data.isFavorited);
            })
            .catch(() => {
